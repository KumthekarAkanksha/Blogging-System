<?php
    ...
    class Queries extends CI_Model {
            ...
            public function login($email, $password){
                $query = $this->db->where(['email'=>$email, 'password'=>$password])->get('tbl_users');
                if($query->num_rows() > 0){
                    return $query->row();
            ...
            public function getSinglePost($postid){
                $query = $this->db->where(['post_id'=> $postid])-> get('tbl_posts');
                if($query->num_rows()>0){
                    return $query->row();
                }
            }
            ...
            public function updateThePost($data, $postid){
                return $this->db->where('post_id', $postid)->update('tbl_posts', $data);
            }
